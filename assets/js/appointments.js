...
function calendario() {
  ...
  for (let i = firstDaysMonth; i > 1; i--) {
    days.innerHTML += `<li class="prev-next-month">${
      lastDaysPrevMonth - i + 1
    }</li>`;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const cita = {
    status: "Espera",
    clients_id: localStorage.getItem("id"),
    services_id: "",
    employees_id: "",
    date: "",
    hora: "",
  ...
  function mostrarMensaje(mensaje, tipo) {
    ...
    if (tipo == "error") {
      ...
      setTimeout(() => {
        window.location.href = `?c=AppointmentsDetails&m=index&id=${localStorage.getItem("id")}`;
      }, 2500);
    }
    setTimeout(() => {
