...
class AppointmentsModel {
    ...
    public function insert($data) {
        ...
        $sql = "SELECT COUNT(*) FROM appointments WHERE employees_id = '{$data['employees_id']}' AND hora = '{$data['hora']}' AND date = '{$data['date']}'";
        $result = $this->svc->query($sql);
        $row = $result->fetch_assoc();
        // return $row;

        if ($row['COUNT(*)'] > 0) {
            return "La fecha y hora ya esta reservada para este empleado.";
        } else {
